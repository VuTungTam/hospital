...
{
    public class UpdateEmployeeProfileCommandHandler : BaseCommandHandler, IRequestHandler<UpdateEmployeeProfileCommand>
    {
        ...
        {
            var profile = await _employeeReadRepository.GetByIdAsync(_executionContext.Identity, cancellationToken: cancellationToken) ?? throw new BadRequestException(_localizer["Account.NotFound"]);
            ...
            //    Category = typeof(Employee).Name.ToSnakeCaseLower(),
            //    Description = $"<p class='audit-normal'>Cập nhật thông tin tài khoản</strong></p>",
            //    AffectedId = profile.Id
            //});

            await _employeeWriteRepository.UpdateAsync(profile, cancellationToken: cancellationToken);
