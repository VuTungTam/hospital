...
namespace Hospital.Application.Dtos.SocialNetworks
{
    public class SocialNetworkDto : BaseDto
    {
        ...
        public class SocialNetworkDtoValidator : BaseAbstractValidator<SocialNetworkDto>
        {
            ...
            {
                RuleFor(x => x.Name).NotEmpty().WithMessage(localizer["social_network_name_is_not_empty"].Value);
                RuleFor(x => x.Link).NotEmpty().WithMessage(localizer["social_network_link_is_not_empty"].Value);
                RuleFor(x => x.Logo).NotEmpty().WithMessage(localizer["social_network_logo_is_not_empty"].Value);
            }
        }
    }
}
