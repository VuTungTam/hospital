...
namespace Hospital.Infrastructure.Repositories.Auth
{
    public class ActionReadRepository : ReadRepository<Action>, IActionReadRepository
    {
        ...
        public async Task<List<ActionWithExcludeValue>> GetActionsByEmployeeIdAsync(long employeeId, CancellationToken cancellationToken = default)
        {
            ...
            var employeeActionTable = new EmployeeAction().GetTableName();
                  ...
                  SELECT a.*, 0 as IsExclude FROM (
                    SELECT t2.*, ra.{nameof(RoleAction.ActionId)} FROM (
                      SELECT t.*, ur.{nameof(EmployeeRole.RoleId)} FROM (
                        SELECT u.Id as EmployeeId FROM {employeeTable} u WHERE Id = {employeeId} AND u.IsDeleted = 0
                      ) t
                      INNER JOIN {employeeRoleTable} ur ON t.EmployeeId = ur.{nameof(EmployeeRole.EmployeeId)} AND ur.IsDeleted = 0
                    ) t2
                    INNER JOIN {roleActionTable} ra ON t2.RoleId = ra.RoleId
                  ) t3
                  INNER JOIN {actionTable} a ON t3.ActionId = a.Id
  
                  UNION
  
                  SELECT a2.*, t5.IsExclude FROM (
                    SELECT t4.EmployeeId, ua2.ActionId, ua2.IsExclude FROM (
                      SELECT u.Id AS EmployeeId FROM {employeeTable} u WHERE Id = {employeeId} AND u.IsDeleted = 0
                    ) t4 
                    INNER JOIN  {employeeActionTable} ua2 ON t4.EmployeeId = ua2.{nameof(EmployeeAction.EmployeeId)}
                  ) t5
                  INNER JOIN {actionTable} a2 ON t5.ActionId = a2.Id
                ) final
            ";
