...
namespace Hospital.SharedKernel.Application.Consts
{
    public class BaseCacheKeys
    {
        ...
        public static string GetCloudinaryCacheFileKey(string fileName) => $"cloudinary:{fileName}";
        ...
        public static string GetForgotsPwdSessionKey(string email) => $"forgot-pwd-session:{email}";

        public static string GetBlockSendForgotPwdKey(string email) => $"block-send-forgot-pwd:{email}";

        

        public static string GetQueueOrder(long serviceId, DateTime dateOnly, TimeSpan start, TimeSpan end) => $"service-queue-order:{serviceId}:{dateOnly}:{start}-{end}";

        public static string GetCurrentOrder(long serviceId) => $"current-order:{serviceId}";
